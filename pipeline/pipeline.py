...
params_opt = fitting.perform_fit(
    0.05, np.log(2), 0.4, 0.15, np.log(8), 0.4, avg_fve_lifetime, rel_int_frac
)
...
params_mono_ps = [
    ...
    8.4,
    sigma_mono,
]  # 8.4 is arbitrary but gives good unimodal approximation
area_mono = fitting.area_under_lognormal(
    params_mono_ps[0], params_mono_ps[2]
)  # this will equal total_area from above

double_lognormal_ps = fitting.double_lognormal(
    avg_fve_lifetime_ps, params_opt_ps
)  # get the bimodal PDF
single_lognormal_ps = fitting.single_lognormal(
    avg_fve_lifetime_ps, params_mono_ps
)  # get the unimodal PDF
...
def print_statistics():
    ...
    print(
        f"c_1={c_1:.3f}, mu_1={mu_1:.3f}, sigma_1={sigma_1:.3f}, c_2={c_2:.3f}, mu_2={mu_2:.3f}, sigma_2={sigma_2:.3f}"
    )
    print(f"AUC_1={area_1:.3f}, AUC_2={area_2:.3f}, AUC_total={total_area:.3f}")
    print(
        f"AUC_1/(AUC_total)={area_1/total_area:.3f}, AUC_2/(AUC_1_total)={area_2/total_area:.3f}"
    )
    ...
    print(
        f"Int_mean_sim = [0.5 0.1 {(1-0.5-0.1)*(area_1/total_area):.3f} {(1-0.5-0.1)*(area_2/total_area):.3f}]"
    )
...
def print_for_plot():
    ...
    for i, val in enumerate(avg_fve_diam):
        print(
            val,
            rel_int_frac[i],
            avg_fve_lifetime_ps[i],
            double_lognormal_ps[i],
            single_lognormal_ps[i],
        )
