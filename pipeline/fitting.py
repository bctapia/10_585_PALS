...
def double_lognormal(x, params):
    ...
    ln_x = np.log(x)
    y = (c1/x*np.exp(-(ln_x-mu1)**2.0/(2*sigma1**2))+c2/x*np.exp(-(ln_x-mu2)**2.0/(2*sigma2**2)))
...
def convert_timescale(params_opt, timescale):    
    ...
    return [timescale*params_opt[0], params_opt[1]+np.log(timescale), params_opt[2], timescale*params_opt[3], params_opt[4]+np.log(timescale), params_opt[5]]
