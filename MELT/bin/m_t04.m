...
firstchannel=20;
TC=zeros(NDt0+firstchannel+8+pad,Ntausmall);		% initialize TC matrix

%Fsave=zeros(Ntausmall,11);
%fsave=zeros(Ntausmall,11);
gridtime=psperchannel*[-firstchannel:NDt0+10+pad];
gridlambda=1./(exp(const+[1:Ntausmall]/increment));
lambdat=gridtime'*gridlambda;
lambda=ones(NDt0+firstchannel+1+10+pad,1)*gridlambda;
...
TC=TC+FWHMint(m)*(Ydivlambda(1:NDt0+firstchannel+8+pad,:)-Ydivlambda(2:NDt0+firstchannel+1+8+pad,:)-tdivlambda(1:NDt0+firstchannel+8+pad,:)+tdivlambda(2:NDt0+firstchannel+1+8+pad,:));
