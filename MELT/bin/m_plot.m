...
% 
% if ~isempty(jf2save)
%     % Optional: reduce f2save based on jf2save if needed
%     % selected_f2 = f2save(:, [1:min(jf2save)-1, max(jf2save)+1:end]);
%     % surf(flipud(selected_f2'))
% 
%     % Full surface plot
%     surf(flipud(f2save'), 'EdgeColor', 'none')
%     colormap('jet')
%     shading interp
%     xlabel('log(TAU)')
%     ylabel('log(entweight)')
%     zlabel('value')
% else
%     surf(flipud(f2save'), 'EdgeColor', 'none')
%     colormap('jet')
