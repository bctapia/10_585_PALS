...
namedat='bimodal_spectrum';	% datafile+variable name
stopdat=7000; % last bin of data
startbg=6900; % first bin for background calculation
...
use_tail_fit_model = 0; % 0 = false, 1 = true
...
sim_smoothness = 10000; % defines the smoothness of the lifetime grid in the simulation mode

countsInSpectrum = 5000000; % number of counts in the spectrum
constBkgrd = 15; % constant background of the spectrum

% BIMODAL DATA FROM pipeline.pyraw_data/Joo_et_al_Figure10a.png
%tau_mean_sim = [4.787 5.940 7.688 8.821]; % mean of the Gaussian distributed charact. lifetimes
%tau_stddev_sim = [0.1 0.1 0.592 0.173]; % standard deviations of the Gaussian distributed charact. lifetimes
%Int_mean_sim = [0.5 0.1 0.104 0.296]; % contributions of the Gaussian distributed charact. lifetimes (must equal to 1.0)
% UNIMODAL DATA FROM pipeline.pyraw_data/Joo_et_al_Figure10a.png
tau_mean_sim = [4.787 5.940 8.400]; % mean of the Gaussian distributed charact. lifetimes
tau_stddev_sim = [0.1 0.1 0.354]; % standard deviations of the Gaussian distributed charact. lifetimes
Int_mean_sim = [0.5 0.1 0.400]; % contributions of the Gaussian distributed charact. lifetimes (must equal to 1.0)
...
%sourcetime=[380];	% source correction lifetimes in ps
%sourceint=[.15];	% source correction intensities

cutoff=1e-9; % singular value cutoff
entwghtstart=5e-6; % entropy weight for initial convergence
%entwghtstart=5e-4; % entropy weight for initial convergence
entwghtstop=1e-7; % lower limit for entropy weight (moderate, relaxed, tight) (4e-7, 7e-7, 3e-7) (3e-7, 4.5e-7, 2E-7) (3e-7, 4.5e-7, 2E-7) (??, ??, 1e-8)  OLDIRF: (4e-7, 3.5e-7, 4e-7) NEWIRF: (4e-7)

cut_off_chi2 = 0.8; % minimum chi-square value used to stop the optimization
...
left_of_max=3 ;	% first bin of data. start left_of_max bins before the maximum. default value=3.
...
Ntau=120; % define analysis lifetime range def 160
			    % should be <= ntau in the matrix TC!
const=1; % determines the first lifetime on the analysis grid 4
increment=12; % determines the spacing of the lifetimes on this grid % 29

ll_tau = 70.; % [ps] % offset of the lifetime to be added to 'const', i.e. the lifetimes where the tc_mat actually starts
